...
class DCache(Cache):
    ...
    def __init__(self, assoc, ceaser, size, locked_lru):
        self.size = size
        self.assoc = assoc
        super().__init__()
        if ceaser == "ceaser":
        ...
        if locked_lru:
            self.replacement_policy = LockedLRURP()
        else:
...
class JustDCacheHierarchy(AbstractClassicCacheHierarchy):
    def __init__(self, assoc=1, ceaser="", cache_size="16KiB", locked_lru=False):
        super().__init__()
    ...
    def incorporate_cache(self, board: AbstractBoard):
        ...
        self.l1dcache = DCache(self._assoc, self._ceaser, self._cache_size, self._locked_lru)
