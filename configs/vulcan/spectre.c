...
/* Edge-case byte values -- exercises value-dependent blind spots that an
 * ASCII phrase (all printable, high bit clear, no long runs of identical
 * bits) never touches: zero, all-ones, each single bit set individually,
 * near-saturated (0x7F/0xFE), and two alternating-bit patterns. Selected
 * via the 3rd CLI arg (target=1). */
uint8_t edge_secret[] = {0x00, 0xFF, 0x01, 0x02, 0x04, 0x08, 0x10,
                          0x20, 0x40, 0x80, 0x7F, 0xFE, 0x33, 0xCC};
int edge_secret_len = sizeof(edge_secret);
...
{
    int target = 0; /* 0 = ASCII secret string, 1 = edge_secret buffer */
    uint8_t *target_buf;
    int i, score[2], len, requested_len;
    size_t malicious_x;
    uint8_t value[2];
    int successes = 0;
    ...
    target_buf = target ? edge_secret : (uint8_t *)secret;
    len = requested_len = target ? edge_secret_len : 20;
    malicious_x = (size_t)((char *)target_buf - (char *)array1);

    if (argc > 1) {
        len = requested_len = atoi(argv[1]);
    }
    if (argc > 2) {
    ...
    while (--len >= 0) {
        ...
        if (score[0] >= 2 * score[1] && value[0] == target_buf[len]) {
            successes++;
        }
        if (score[1] > 0) {
