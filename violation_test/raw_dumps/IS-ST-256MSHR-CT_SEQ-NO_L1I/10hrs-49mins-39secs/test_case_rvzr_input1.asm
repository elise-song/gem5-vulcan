.intel_syntax noprefix
LEA R14, [R14 + 60] # instrumentation
MFENCE # instrumentation
.test_case_enter:
.function_main:
.bb_main.entry:
JMP .bb_main.0 
.bb_main.0:
AND RDX, 0b111111111111 # instrumentation
LOCK AND qword ptr [R14 + RDX], RBX 
AND RDX, 0b111111111111 # instrumentation
CMOVNP EDX, dword ptr [R14 + RDX] 
JNP .bb_main.1 
JMP .bb_main.exit 
.bb_main.1:
AND RAX, 0b111111111111 # instrumentation
XOR byte ptr [R14 + RAX], 100 
AND RCX, 0b111111111111 # instrumentation
LOCK AND dword ptr [R14 + RCX], 63 
AND RDI, 0b111111111111 # instrumentation
LOCK OR byte ptr [R14 + RDI], AL 
AND RSI, 0b111111111111 # instrumentation
LOCK AND qword ptr [R14 + RSI], -13 
JNBE .bb_main.2 
JMP .bb_main.4 
.bb_main.2:
AND DL, 71 # instrumentation
AND RBX, 0b111111111111 # instrumentation
CMOVBE RBX, qword ptr [R14 + RBX] 
JMP .bb_main.3 
.bb_main.3:
AND DL, 19 # instrumentation
AND RSI, 0b111111111111 # instrumentation
CMOVBE ECX, dword ptr [R14 + RSI] 
AND RCX, 0b111111111111 # instrumentation
CMOVNO BX, word ptr [R14 + RCX] 
AND RBX, 0b111111111111 # instrumentation
LOCK OR qword ptr [R14 + RBX], 55 
AND RDI, 0b111111111111 # instrumentation
OR word ptr [R14 + RDI], 0b1000000000000000 # instrumentation
BSR CX, word ptr [R14 + RDI] 
JMP .bb_main.4 
.bb_main.4:
AND DL, 113 # instrumentation
AND RDX, 0b111111111111 # instrumentation
CMOVBE CX, word ptr [R14 + RDX] 
AND RSI, 0b111111111111 # instrumentation
CMOVBE EDI, dword ptr [R14 + RSI] 
AND RCX, 0b111111111111 # instrumentation
CMOVNB EBX, dword ptr [R14 + RCX] 
AND RCX, 0b111111111111 # instrumentation
LOCK NOT dword ptr [R14 + RCX] 
AND RDX, 0b111111111111 # instrumentation
LOCK AND word ptr [R14 + RDX], DI 
.bb_main.exit:
.test_case_exit:
MFENCE # instrumentation
LEA R14, [R14 - 60] # instrumentation
